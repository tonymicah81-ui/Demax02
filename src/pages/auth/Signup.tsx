...
export default function Signup() {
  ...
  const inputClass = "w-full bg-slate-50 dark:bg-slate-800/50 border border-brand-border dark:border-white/5 rounded-xl pl-12 pr-4 py-4 text-sm font-bold focus:outline-none focus:border-brand-accent transition-all dark:text-white";

  return (
    <div className="min-h-screen bg-brand-bg dark:bg-slate-950 flex items-center justify-center p-4 font-sans text-brand-text transition-colors duration-500 py-8">
      <motion.div
      ...
      >
        <div className="bg-brand-primary p-8 sm:p-12 text-center border-b border-brand-border dark:border-white/5 relative">
          <div className="flex items-center gap-2 justify-center mb-5">
            <Logo size="lg" className="bg-white/10 p-3 rounded-2xl" showText={false} />
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tighter uppercase italic leading-tight">Create Account</h1>
        ...
        <div className="p-6 sm:p-10 space-y-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={inputClass + " placeholder:normal-case placeholder:font-normal"}
            ...
            {error && (
              <motion.div
                ...
                className="p-4 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 text-red-600 dark:text-red-400 text-xs font-bold rounded-xl"
              >
                {error}
              </motion.div>
            )}
            ...
            <div className="pt-2">
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-14 bg-brand-primary dark:bg-brand-accent hover:opacity-90 transition-all text-sm font-black shadow-xl"
              >
          ...
          <div className="pt-4 border-t border-brand-border dark:border-white/5 text-center space-y-3">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Already have an account?</p>
            <Link
              to="/login"
              className="inline-block text-[11px] font-black text-brand-accent hover:underline uppercase tracking-[0.2em]"
            >
              Sign In
            </Link>
          </div>

...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Authentication rejected. Protocol failure.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-brand-bg dark:bg-slate-950 flex items-center justify-center p-4 font-sans text-brand-text transition-colors duration-500">
      ...
      >
        <div className="bg-brand-primary p-12 text-center border-b border-brand-border dark:border-white/5 relative">
          ...
          <h1 className="text-3xl font-black text-white tracking-tighter uppercase italic">Identity Link</h1>
          <p className="text-slate-400 mt-2 text-xs font-mono uppercase tracking-[0.3em]">Authorized Access v1.2</p>
        ...
        <div className="p-10 space-y-8">
            <form onSubmit={handleLogin} className="space-y-6">
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">ID Relay / Email</label>
               ...
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">Frequency Lock / Password</label>
                 <div className="relative">
               ...
               >
                 {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "AUTHENTICATE SIGNAL_"}
               </Button>
            </form>
            ...
            <div className="pt-8 border-t border-brand-border dark:border-white/5 text-center">
               <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 italic">No Protocol Active?</p>
               <Link 
                 to="/signup"
                 className="text-[11px] font-black text-brand-accent hover:underline uppercase tracking-[0.2em] italic"
               >
                 &gt; Initialize Core Registration
               </Link>
            </div>
        </div>
      </motion.div>

...
import { db, collection, onSnapshot, doc, updateDoc, serverTimestamp, query, orderBy, addDoc } from "../../firebase";
  ...
  const updateStatus = async (fixId: string, userId: string, newStatus: Fix["status"]) => {
    ...
    try {
      ...
      await addDoc(collection(db, "user_notifications"), {
        ...
        message: `Your project fix request has been updated to: ${newStatus.toUpperCase()}.`,
        read: false,
              ...
              <Card key={f.id} className="group hover:border-brand-accent/30 transition-all border-none bg-white dark:bg-slate-900 shadow-md">
                <div className="flex flex-col lg:flex-row items-center justify-between gap-8 p-2">
                  <div className="flex items-center gap-6 flex-1">
                    <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center border border-brand-border dark:border-white/5 shadow-sm relative overflow-hidden shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-500 font-medium italic mt-1">
                        {f.createdAt ? new Date(f.createdAt.seconds * 1000).toLocaleString() : "N/A"}
                      </p>
                    </div>
                  </div>
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              f.status === st
                                ...
                                : "bg-slate-100 dark:bg-slate-800 border-transparent text-slate-400 hover:border-brand-accent/30"
                            )}
      ...
      <div className="p-8 bg-slate-50 dark:bg-slate-900 rounded-3xl border border-brand-border dark:border-white/5 flex gap-6 items-start">
        ...
        <div className="space-y-4">
          ...
          <p className="text-[11px] text-slate-500 font-medium leading-relaxed uppercase">
            Fix requests are submitted by users when a project reaches the{" "}
            <span className="text-brand-success font-black">DELIVERED</span> status. Admins review and update the status manually before marking as{" "}
            <span className="text-brand-success font-black">COMPLETED</span>.
          </p>
        </div>
      </div>
    </div>

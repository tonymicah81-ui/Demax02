...
export default function ManagePayments() {
  ...
  useEffect(() => {
    ...
    // History Stream (Simplified for this view)
      ...
      if (action === "approve") {
        await runTransaction(db, async (tx) => {
          ...
          tx.set(notifRef, {
            ...
            message: `Your deposit of $${payment.amount.toFixed(2)} has been verified. Balances synchronized.`,
            read: false,
        ...
        await addDoc(collection(db, "user_notifications"), {
          ...
          message: `Your deposit of $${payment.amount.toFixed(2)} was rejected due to verification divergence.`,
          read: false,
          createdAt: serverTimestamp()
        });
    ...
    <div className="space-y-10">
      ...
      <div className="grid lg:grid-cols-4 gap-8">
         <Card className="lg:col-span-1 bg-brand-primary text-white border-none shadow-2xl space-y-6">
            <div>
               <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">Total Assets Ingested</p>
               <p className="text-4xl font-black italic tracking-tighter">${totalDeposits.toLocaleString()}</p>
            </div>
            <div className="pt-6 border-t border-white/10 space-y-4">
               <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase opacity-60">Awaiting Action</span>
               ...
               <div className="flex justify-between items-center text-brand-success">
                  <span className="text-[9px] font-bold uppercase">Success Rate</span>
               ...
               <div className="grid gap-4">
                 {loading ? (
                   ...
                   <div className="h-64 flex flex-col items-center justify-center space-y-4 border border-dashed border-brand-border rounded-3xl opacity-30">
                      ...
                      <p className="text-sm font-black uppercase tracking-widest italic">All Transactions Synchronized</p>
                   </div>
                 ) : (
                   payments.map((p) => (
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-8 p-2">
                           ...
                           <div className="flex items-center gap-8">
                              <div className="text-right">
                                 ...
                                 <button onClick={() => setViewProof(p.proofUrl)} className="text-[9px] font-black text-brand-accent uppercase tracking-widest flex items-center gap-1 hover:underline mt-1"><ExternalLink className="w-3 h-3" /> Proof</button>
                              </div>
               ...
               <Card className="space-y-6">
                  <div className="flex items-center justify-between">
                     ...
                     <div className="relative">
                        ...
                        <input 
                          type="text" 
                          placeholder="Search Sequence ID / Node..." 
                          value={searchQuery}
                  ...
                  <div className="space-y-3">
                     {filteredHistory.map(h => (
                       <div key={h.id} className="p-4 rounded-xl border border-brand-border dark:border-white/5 flex items-center justify-between group">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                <p className="text-[10px] font-black text-brand-text-bold dark:text-white uppercase italic tracking-tighter">{h.status === 'completed' ? 'Authorized' : 'Purged'} Deposit</p>
                     ...
                     {filteredHistory.length === 0 && (
                        <div className="py-20 text-center opacity-20">
                           ...
                           <p className="text-[10px] font-black uppercase tracking-widest">No Log Fragments Found</p>
                        </div>
                     )}
                  </div>
               </Card>
         ...
         {viewProof && (
           <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl flex items-center justify-center z-50 p-6" onClick={() => setViewProof(null)}>
              <div className="relative max-w-4xl w-full h-[80vh] bg-slate-900 rounded-3xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
                 <img src={viewProof} className="w-full h-full object-contain" alt="Proof" />

...
import {
  Monitor, Rocket, Layout, Shield, ChevronRight, Globe,
  Layers, MessageSquare, Zap, CheckCircle2, Moon, Sun,
  Server, Cloud, Cpu, Database, ArrowUpRight, ShoppingCart, Loader2
} from "lucide-react";
...
export default function LandingPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "products"), (snap) => {
      ...
      const display = featured.length >= 3 ? featured.slice(0, 6) : all.slice(0, 6);
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="text-lg text-brand-text dark:text-slate-400 mb-8 max-w-lg leading-relaxed font-medium">
              We build high-performance web applications starting at{" "}
              <span className="font-bold text-brand-text-bold dark:text-white underline decoration-brand-accent/30 tracking-tighter">$60.00</span>.
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div>
              ...
              <h3 className="text-4xl font-bold text-brand-text-bold dark:text-white mb-6 leading-tight">
                Why is our pricing <span className="text-brand-success underline decoration-brand-success/20 italic font-black">unbeatable</span>?
              </h3>
            ...
            <motion.div style={{ rotateZ: 2 }} className="bg-brand-primary p-1 rounded-3xl overflow-hidden shadow-2xl relative">
      ...
      <section id="services" className="py-24 px-6 relative">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 border-b border-brand-border pb-12">
            <div>
              ...
              <p className="text-brand-text dark:text-slate-400 mt-2 max-w-xl font-medium opacity-80 italic">
                Professional-grade web products ready for instant deployment.
              </p>
            </div>
            <button
              onClick={() => window.location.href = "/store"}
              className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-brand-accent hover:opacity-80 transition-opacity shrink-0"
            >
          ...
          {loadingProducts ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-brand-border dark:bg-white/5 border border-brand-border dark:border-white/5 overflow-hidden rounded-xl">
              {featuredProducts.map((product, i) => (
                <motion.div
                  ...
                  className="p-8 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all group relative cursor-pointer"
                  onClick={() => window.location.href = "/store"}
                >
                  {product.images?.[0] ? (
                    <div className="w-12 h-12 rounded-xl overflow-hidden mb-6 border border-brand-border dark:border-white/5 shadow-sm">
                      <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
                    </div>
                  ) : (
                    <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 text-brand-text-bold dark:text-white flex items-center justify-center rounded-xl mb-6 group-hover:bg-brand-accent group-hover:text-white transition-all shadow-sm">
                      <ShoppingCart className="w-6 h-6" />
                    </div>
                  )}
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-lg font-bold text-brand-text-bold dark:text-white tracking-tight">{product.name}</h3>
                    <span className="text-brand-success font-black italic text-lg ml-2 shrink-0">${product.price}</span>
                  </div>
                  <p className="text-sm text-brand-text dark:text-slate-500 leading-relaxed font-medium mb-6 line-clamp-2">{product.description}</p>
                  <div className="pt-4 border-t border-slate-100 dark:border-white/5 flex items-center gap-2 text-brand-accent text-xs font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                    View in Store <ArrowUpRight className="w-4 h-4" />
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-brand-border dark:bg-white/5 border border-brand-border dark:border-white/5 overflow-hidden rounded-xl">
              {[
                ...
                >
                  ...
                  <div className="pt-6 border-t border-slate-100 dark:border-white/5 flex items-center gap-2 text-brand-accent text-xs font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                    View in Store <ArrowUpRight className="w-4 h-4" />
                  </div>
                </motion.div>
              ))}
          ...
          <div className="mt-12 text-center">
            <button
              onClick={() => window.location.href = "/store"}
              className="inline-flex items-center gap-2 bg-brand-primary dark:bg-brand-accent text-white px-8 py-4 rounded-xl font-bold hover:opacity-90 transition-all shadow-xl shadow-brand-accent/20 text-sm"
            >
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center gap-12 mb-12">
            ...
            <div className="flex gap-10 text-xs font-bold text-slate-400 uppercase tracking-widest">
              ...
              <a href="/store" className="hover:text-brand-accent transition-colors">Store</a>
